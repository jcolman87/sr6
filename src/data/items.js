	...
	static defineSchema() {
		...
		return {
			...
			locations: new fields.ArrayField(
				new fields.StringField({choices: () => { let choices = [];Object.keys(Enums.WeaponAttachmentLocation).filter((v) => isNaN(Number(v))).forEach((key, index) => choices.push(key));return choices; }}),
			)
		}
	}
}
