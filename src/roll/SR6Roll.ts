...
export class SR6Roll extends Roll {
	...
	get pool(): number {
		return (this.terms[0] as any).results.length;
	}
	get sixes(): number {
	...
	get hits(): number {
		let hits: number = this.sides.reduce((hits, result) => (this.options.parameters.success.includes(result) ? hits + 1 : hits), 0);
	...
	async rerollFailed(): Promise<boolean> {
		...
		let reroll = new SR6Roll(`${failedList.length}d6`);
		reroll.evaluate({ async: false });
		await reroll.showVisual();
	...
	async explode() {
		if (this.sixes > 0) {
			let explode_roll = new SR6Roll(`${this.sixes}d6`);
	...
	_termsUpdated() {
		(this.terms[0] as any).number = (this.terms[0] as any).results.length;
	}
