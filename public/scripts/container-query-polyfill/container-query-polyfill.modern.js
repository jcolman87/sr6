if (!('container' in document.documentElement.style)) {
	console.debug('SR6 | CSS Container Query support not found, loading polyfill...');
	function e() {
		return (
			(e = Object.assign
				? Object.assign.bind()
	...
	const f = Array.from({ length: 4 }, () => Math.floor(256 * Math.random()).toString(16)).join(''),
		p = S('container'),
		y = S('container-type'),
		h = S('container-name'),
		v = `data-cqs-${f}`,
	...
	function* $(e) {
		...
		function p() {
			const [e, t] = (function () {
					...
					if (((n = u(1)), 69 === n || 101 === n)) {
						...
						else if ((45 === n || 43 === n) && k(u(3))) for (e = 1, o(1), t += c(), o(1), t += c(); k(u(1)); ) o(1), (t += c());
					}
					return [t, e];
				})(),
				n = u(1);
		...
		for (;;) {
			...
			else if (60 === e) 33 === u(1) && 45 === u(2) && 45 === u(3) ? yield { type: 18 } : yield i();
			else if (64 === e)
				if (d(u(1), u(2), u(3))) {
					const e = w();
	...
	function D(e, t) {
		switch (e.type) {
			case 25:
				return `@${CSS.escape(e.name)} ${e.prelude.map((e) => D(e)).join('')}${e.value ? D(e.value) : ';'}`;
			case 26:
				return `${e.prelude.map((e) => D(e)).join('')}${D(e.value)}`;
			case 28: {
				const [t, n] = _[e.source.type];
				return `${t}${W(e.value)}${n}`;
			}
			case 27:
				return `${CSS.escape(e.name)}(${e.value.map((e) => D(e)).join('')})`;
			case 29:
				return `${CSS.escape(e.name)}:${e.value.map((e) => D(e)).join('')}${e.important ? ' !important' : ''}`;
			case 1:
				return ' ';
			case 8:
				return ';';
	...
	const ce = { width: 1, height: 2, 'inline-size': 3, 'block-size': 4, 'aspect-ratio': 5, orientation: 6 },
			...
			case 'container':
				return (function (t, n) {
					if (t.value) {
						const r = $e(t.prelude);
						if (r !== x) {
							const u = { rule: r, selector: null, parent: n.parent, uid: 'c' + Ae++ },
								o = new Set(),
								s = [],
									...
									transformStyleRule: (t) => {
										const [n, r] = (function (e, t, n) {
											...
											for (;;) {
												...
												let y = f,
													h = [{ type: 28, source: { type: 9 }, value: { type: 0, value: [ne(p.length > 0 ? v : d), ee('~'), ee('='), { type: 2, value: t }] } }];
	...
	function _e(e, t) {
		...
		if (typeof e == typeof t && null !== e && null !== t && 'object' == typeof e) {
			if (Array.isArray(e)) {
				...
				for (let n = 0, r = e.length; n < r; n++) if (!_e(e[n], t[n])) return !1;
				return !0;
			}
	...
	const De = Symbol('CQ_INSTANCE'),
		We = Symbol('CQ_STYLESHEET'),
		Be = CSS.supports('width: 1svh'),
		Ge = new Set(['vertical-lr', 'vertical-rl', 'sideways-rl', 'sideways-lr', 'tb', 'tb-lr', 'tb-rl']),
		Ye = ['padding-left', 'padding-right', 'border-left-width', 'border-right-width'],
		Je = ['padding-top', 'padding-bottom', 'border-top-width', 'border-bottom-width'],
		Ke = /(\w*(\s|-))?(table|ruby)(-\w*)?/;
	class Xe {
		constructor(e) {
				...
				n.origin === location.origin &&
					(this.controller = rt(async function (r) {
						const u = await fetch(n.toString(), { signal: r }),
							o = await u.text(),
							s = (e.styleSheet = await e.context.registerStyleSheet({ source: o, url: n, signal: r })),
	...
	class et extends Xe {
		...
		connected() {
			...
			this.controller = rt(async function (t) {
				const n = e.node,
					r = (e.styleSheet = await e.context.registerStyleSheet({ source: n.innerHTML, signal: t }));
	...
	function it(e) {
		return {
			...
			containerNames: ((n = e(h).trim()), n.startsWith('cq-') && ('none' === (n = n.substring('cq-'.length)) || ge(n)) ? new Set([]) : new Set(0 === n.length ? [] : n.split(' '))),
	...
	new Promise((e) => {}),
		(window.CQPolyfill = { version: '${npm_package_version}' }),
			...
			(function (n) {
				...
				new MutationObserver((e) => {
					for (const t of e) {
						...
						(t.target.nodeType !== Node.DOCUMENT_NODE && t.target.nodeType !== Node.DOCUMENT_FRAGMENT_NODE && null === t.target.parentNode) ||
							('attributes' === t.type && t.attributeName && (t.attributeName === v || t.attributeName === d || (t.target instanceof Element && t.target.getAttribute(t.attributeName) === t.oldValue))) ||
							(A(t.target).mutate(), S());
					}
				...
				async function a(e, { source: t, url: n, signal: r }) {
					const o = (function (e, t) {
						try {
							...
							return { source: W(Oe(P(n, !0), r)), descriptors: r.descriptors };
						} catch (t) {
								...
								if (r) {
									...
									if (z) {
										...
										let u = null,
											...
											s = null,
											l = null;
										(r !== e.context.writingAxis || t.isQueryContainer) && ((u = `var(${0 === r ? m : w})`), (o = `var(${1 === r ? m : w})`)),
											(e && !t.isQueryContainer) || (n.cqw && (s = n.cqw + 'px'), n.cqh && (l = n.cqh + 'px')),
											at(z, g, u),
											at(z, b, o),
											at(z, m, s),
											at(z, w, l);
